...
export default function App() {
  ...
  // Kişiyi ve kayıtlarını tamamen sil
  ...
  // Aktif kişilerin isimlerini al
  const aktifKisiIsimleri = kisiler.map(k => k.isim_soyisim);

  // Sadece aktif kişilerin kayıtlarını filtrele
  ...
  function mahalleBazliRapor() {
    ...
    return mahalleler.map(mahalle => {
      ...
      return {
        ...
        anaKademe: mahalleKisiler.filter(k => k.kategori === "mahalle").map(k => k.isim_soyisim),
        kadinKollari: mahalleKisiler.filter(k => k.kategori === "kadin_mahalle").map(k => k.isim_soyisim),
        hedef: toplamHedef,
        ...
        <div style={styles.sideNav}>
          {[
            ...
            <button key={item.key} style={{ ...styles.sideNavBtn, background: screen === item.key ? "rgba(244,166,32,0.15)" : "transparent", color: screen === item.key ? "#F4A620" : "#8A9BB5", borderLeft: screen === item.key ? "3px solid #F4A620" : "3px solid transparent" }}
              onClick={() => { setScreen(item.key); setSeciliMahalle(null); if (item.key === "liste" || item.key === "rapor" || item.key === "mahalleler") yukle(kullanici); if (item.key === "kullanicilar") kullanicilariYukle(); if (item.key === "form") iptalDuzenle(); }}>
          ...
          <div style={styles.card}>
            ...
            {duzenleKayit && (
              <div style={{ background: "#e3f2fd", padding: 12, borderRadius: 8, marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 13 }}>📝 <strong>{duzenleKayit.isim_soyisim}</strong> - {duzenleKayit.tutanak_tarih}</span>
                ...
                {Object.entries(KAT_LABELS).map(([key, val]) => (
                  <optgroup key={key} label={`${val.icon} ${val.label}`}>
                    {kisilerByKategori(key).map(k => (
                      <option key={k.id} value={k.isim_soyisim}>{k.isim_soyisim}{k.mahalle ? ` (${k.mahalle})` : ""}</option>
                    ))}
                  </optgroup>
                ))}
              </select>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <input style={styles.input} placeholder="Özel not..." value={form.notlar} onChange={e => setForm(p => ({ ...p, notlar: e.target.value }))} />
            </div>
          ...
          <div>
            ...
            {!isAdmin && (
              <div style={styles.summaryRow}>
                <div style={styles.summaryCard}><div style={styles.summaryVal}>{aktifKayitlar.reduce((s, k) => s + (k.teslim_edilen || 0), 0)}</div><div style={styles.summaryLbl}>Teslim</div></div>
                <div style={styles.summaryCard}><div style={{ ...styles.summaryVal, color: "#2e7d32" }}>{aktifKayitlar.reduce((s, k) => s + (k.yeni_uye || 0), 0)}</div><div style={styles.summaryLbl}>Yeni Üye</div></div>
                <div style={styles.summaryCard}><div style={{ ...styles.summaryVal, color: "#c0392b" }}>{aktifKayitlar.reduce((s, k) => s + (k.mukerrer || 0), 0)}</div><div style={styles.summaryLbl}>Mükerrer</div></div>
              ...
              <div>
                {!isMobile ? (
                  <table style={styles.table}>
                        ...
                        return (
                          <tr key={k.isim} style={{ background: k.kayit === 0 ? "#FFF8E1" : hedefDurumu === "tamamlandi" ? "#e8f5e9" : "#fff" }}>
                            ...
                            <td style={{ ...styles.td, textAlign: "center" }}>
                              {hedefDuzenle === k.id ? (
                                <input type="number" min="0" style={{ width: 60, padding: 4, textAlign: "center", border: "1px solid #ddd", borderRadius: 4 }} 
                                  defaultValue={k.hedef} autoFocus
                            ...
                            <td style={styles.td}>
                              <button style={{ fontSize: 14, background: "transparent", border: "none", cursor: "pointer" }} onClick={() => kisiSil(k.id)}>🗑️</button>
                            </td>
                          </tr>
                        );
                      })}
                    ...
                    <tfoot>
                      <tr style={{ background: "#1A2942", color: "#fff" }}>
                        ...
                        <td style={{ ...styles.td, textAlign: "center" }}>{kisiOzeti.reduce((s, k) => s + k.hedef, 0)}</td>
                        <td style={{ ...styles.td, textAlign: "center", fontWeight: 700, color: "#F4A620" }}>{kisiOzeti.reduce((s, k) => s + k.yeni, 0)}</td>
                        <td style={{ ...styles.td, textAlign: "center" }}>{kisiOzeti.reduce((s, k) => s + k.hedef, 0) > 0 ? `%${Math.round((kisiOzeti.reduce((s, k) => s + k.yeni, 0) / kisiOzeti.reduce((s, k) => s + k.hedef, 0)) * 100)}` : "—"}</td>
                  ...
                  <div>
                    {kisiOzeti.map(k => {
                      const ilerlemeYuzde = k.hedef > 0 ? Math.min(100, Math.round((k.yeni / k.hedef) * 100)) : 0;
                      const hedefDurumu = k.hedef > 0 ? (k.yeni >= k.hedef ? "tamamlandi" : "devam") : "yok";
                      return (
                        <div key={k.isim} style={{ ...styles.personCard, background: hedefDurumu === "tamamlandi" ? "#e8f5e9" : "#fff" }} onClick={() => setSeciliKisi(seciliKisi === k.isim ? null : k.isim)}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            ...
                            <div style={{ textAlign: "right" }}>
                              <div style={{ fontSize: 20, fontWeight: 700, color: "#2e7d32" }}>{k.yeni}{k.hedef > 0 && <span style={{ fontSize: 12, color: "#888", fontWeight: 400 }}>/{k.hedef}</span>}</div>
                              {k.hedef > 0 && (
                                <div style={{ width: 50, height: 4, background: "#e0e0e0", borderRadius: 2, marginTop: 4, marginLeft: "auto" }}>
                                  <div style={{ width: `${ilerlemeYuzde}%`, height: "100%", background: hedefDurumu === "tamamlandi" ? "#2e7d32" : "#F4A620", borderRadius: 2 }}></div>
                                </div>
                              )}
                            </div>
                          </div>
                          {seciliKisi === k.isim && (
                            <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #eee" }}>
                              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 12, marginBottom: 10 }}>
                                ...
                                <div>İlerleme: <strong>{k.hedef > 0 ? `%${ilerlemeYuzde}` : "—"}</strong></div>
                    ...
                    <div style={styles.totalBar}>
                      ...
                      <div style={{ display: "flex", gap: 16, marginTop: 8, fontSize: 12 }}>
                        <span>Hedef: <strong>{kisiOzeti.reduce((s, k) => s + k.hedef, 0)}</strong></span>
                  ...
                  <div key={k.id} style={{ background: "#fff", borderRadius: 10, padding: 14, marginBottom: 8, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                    ...
                    {isAdmin && (
                      <div style={{ marginTop: 10, display: "flex", justifyContent: "flex-end", gap: 8 }}>
                        ...
                        {silOnay === k.id ? (
                          <>
                            <button style={{ fontSize: 12, background: "#c0392b", color: "#fff", border: "none", borderRadius: 4, padding: "6px 10px", cursor: "pointer" }} onClick={() => sil(k.id)}>Sil</button>
            ...
            <div style={styles.card}>
              ...
              {!seciliMahalle ? (
                  ...
                  {!isMobile ? (
                    <table style={styles.table}>
                      <thead>
                        <tr>
                          <th style={styles.th}>Mahalle</th>
                          <th style={styles.th}>Ana Kademe</th>
                          <th style={styles.th}>Kadın Kolları</th>
                          <th style={{ ...styles.th, textAlign: "center" }}>Hedef</th>
                          <th style={{ ...styles.th, textAlign: "center", color: "#2e7d32" }}>Yeni Üye</th>
                          <th style={{ ...styles.th, textAlign: "center" }}>İlerleme</th>
                        </tr>
                      ...
                      <tbody>
                        {mahalleRaporu.map(m => {
                          ...
                          const hedefDurumu = m.hedef > 0 ? (m.yeni >= m.hedef ? "tamamlandi" : m.yeni >= m.hedef * 0.7 ? "yaklasti" : "devam") : "yok";
                          return (
                            <tr key={m.mahalle} style={{ background: hedefDurumu === "tamamlandi" ? "#e8f5e9" : "#fff", cursor: "pointer" }} onClick={() => setSeciliMahalle(m.mahalle)}>
                              <td style={{ ...styles.td, fontWeight: 600 }}>{m.mahalle}</td>
                              <td style={{ ...styles.td, fontSize: 11 }}>{m.anaKademe.join(", ") || "—"}</td>
                              <td style={{ ...styles.td, fontSize: 11 }}>{m.kadinKollari.join(", ") || "—"}</td>
                              <td style={{ ...styles.td, textAlign: "center" }}>{m.hedef || "—"}</td>
                              <td style={{ ...styles.td, textAlign: "center", fontWeight: 700, color: "#2e7d32" }}>{m.yeni}</td>
                              <td style={{ ...styles.td, textAlign: "center" }}>
                                {m.hedef > 0 ? (
                      ...
                      <tfoot>
                        <tr style={{ background: "#1A2942", color: "#fff" }}>
                          <td style={{ ...styles.td, fontWeight: 700 }} colSpan={3}>TOPLAM ({getMahalleler().length} mahalle)</td>
                          <td style={{ ...styles.td, textAlign: "center" }}>{mahalleRaporu.reduce((s, m) => s + m.hedef, 0)}</td>
                          <td style={{ ...styles.td, textAlign: "center", fontWeight: 700, color: "#F4A620" }}>{mahalleRaporu.reduce((s, m) => s + m.yeni, 0)}</td>
                          <td style={{ ...styles.td, textAlign: "center" }}>
                            {mahalleRaporu.reduce((s, m) => s + m.hedef, 0) > 0 ? `%${Math.round((mahalleRaporu.reduce((s, m) => s + m.yeni, 0) / mahalleRaporu.reduce((s, m) => s + m.hedef, 0)) * 100)}` : "—"}
                          </td>
                        </tr>
                      </tfoot>
                    </table>
                    ...
                    <div>
                      {mahalleRaporu.map(m => {
                        const ilerlemeYuzde = m.hedef > 0 ? Math.min(100, Math.round((m.yeni / m.hedef) * 100)) : 0;
                        const hedefDurumu = m.hedef > 0 ? (m.yeni >= m.hedef ? "tamamlandi" : "devam") : "yok";
                        return (
                          <div key={m.mahalle} style={{ background: hedefDurumu === "tamamlandi" ? "#e8f5e9" : "#f9f9f9", borderRadius: 8, padding: 12, marginBottom: 8, cursor: "pointer" }} onClick={() => setSeciliMahalle(m.mahalle)}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              <div style={{ fontWeight: 700, fontSize: 14 }}>{m.mahalle}</div>
                              <div style={{ textAlign: "right" }}>
                                <div style={{ fontSize: 18, fontWeight: 700, color: "#2e7d32" }}>{m.yeni}{m.hedef > 0 && <span style={{ fontSize: 12, color: "#888", fontWeight: 400 }}>/{m.hedef}</span>}</div>
                                {m.hedef > 0 && (
                                  <div style={{ width: 50, height: 4, background: "#e0e0e0", borderRadius: 2, marginTop: 4, marginLeft: "auto" }}>
                                    <div style={{ width: `${ilerlemeYuzde}%`, height: "100%", background: hedefDurumu === "tamamlandi" ? "#2e7d32" : "#F4A620", borderRadius: 2 }}></div>
                                  </div>
                            ...
                            <div style={{ fontSize: 11, color: "#666", marginTop: 6 }}>
                              {m.anaKademe.length + m.kadinKollari.length} kişi
                            </div>
                          </div>
                        );
                      })}
                      <div style={styles.totalBar}>
                        <div style={{ fontWeight: 700 }}>TOPLAM ({getMahalleler().length} mahalle)</div>
                        <div style={{ display: "flex", gap: 16, marginTop: 6, fontSize: 12 }}>
                          <span>Hedef: <strong>{mahalleRaporu.reduce((s, m) => s + m.hedef, 0)}</strong></span>
                          <span style={{ color: "#F4A620" }}>Yeni: <strong>{mahalleRaporu.reduce((s, m) => s + m.yeni, 0)}</strong></span>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <div>
                  ...
                  <div style={{ fontSize: 18, fontWeight: 700, color: "#1A2942", marginBottom: 16 }}>{seciliMahalle}</div>
                    ...
                    return (
                      <div key={k.id} style={{ background: "#f9f9f9", borderRadius: 8, padding: 12, marginBottom: 8 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <div>
                            <div style={{ fontWeight: 600, fontSize: 14 }}>{k.isim}</div>
                            <div style={{ fontSize: 11, color: "#888" }}>{KAT_LABELS[k.kategori]?.icon} {KAT_LABELS[k.kategori]?.label}</div>
                          </div>
                          <div style={{ textAlign: "right" }}>
                            <div style={{ fontSize: 18, fontWeight: 700, color: "#2e7d32" }}>{k.yeni}{k.hedef > 0 && <span style={{ fontSize: 12, color: "#888", fontWeight: 400 }}>/{k.hedef}</span>}</div>
                          </div>
                        </div>
                        <div style={{ display: "flex", gap: 12, fontSize: 12, marginTop: 8 }}>
                          <span>Teslim: <strong>{k.teslim}</strong></span>
                          {k.muk > 0 && <span style={{ color: "#c0392b" }}>Mük: <strong>{k.muk}</strong></span>}
                          {k.hedef > 0 && <span>İlerleme: <strong>%{ilerlemeYuzde}</strong></span>}
                        </div>
                      </div>
                    );
                  })}
        ...
        {screen === "rapor" && isAdmin && (
          <div>
            ...
            <div style={styles.card}>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#1A2942", marginBottom: 10, borderBottom: "1px solid #eee", paddingBottom: 6 }}>👩 KADIN KOLLARI</div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 10px", background: "#fce4ec", borderRadius: 6, marginTop: 8, fontSize: 13 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 24, width: "100%", maxWidth: 400, maxHeight: "90vh", overflow: "auto" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <select style={styles.input} value={yeniKisi.mahalle} onChange={e => setYeniKisi(p => ({ ...p, mahalle: e.target.value }))}>
                ...
                <option value="__yeni__">+ Yeni Mahalle Ekle</option>
              </select>
              {yeniKisi.mahalle === "__yeni__" && (
                <input style={{ ...styles.input, marginTop: 8 }} placeholder="Yeni mahalle adı" onChange={e => setYeniKisi(p => ({ ...p, mahalle: e.target.value.toUpperCase() }))} />
              )}
            </div>
            ...
            <div style={styles.grid2}>
              <div>
                ...
                <input style={styles.input} placeholder="YK Üyesi vb." value={yeniKisi.gorev} onChange={e => setYeniKisi(p => ({ ...p, gorev: e.target.value }))} />
              </div>
              <div>
                <label style={styles.label}>🎯 Hedef</label>
                <input style={styles.input} type="number" min="0" placeholder="0" value={yeniKisi.hedef} onChange={e => setYeniKisi(p => ({ ...p, hedef: e.target.value }))} />
              </div>
            </div>

            {kisiMesaj && <div style={{ ...styles.msg, marginTop: 16, background: kisiMesaj.startsWith("✅") ? "#e8f5e9" : "#fdecea", color: kisiMesaj.startsWith("✅") ? "#2e7d32" : "#c0392b" }}>{kisiMesaj}</div>}
